...
from hummingbot.connector.exchange.bitstamp import bitstamp_utils
from hummingbot.connector.exchange.bitstamp import bitstamp_constants as CONSTANTS
...
class BitstampExchange(ExchangeBase):
    """
    BitstampExchange connects with Bitstamp exchange and provides order book pricing, user account tracking and
    ...
    def logger(cls) -> HummingbotLogger:
        global bse_logger
        if bse_logger is None:
            bse_logger = logging.getLogger(__name__)
        return bse_logger
    ...
    def __init__(self,
        ...
        """
        :param bitstamp_api_key: The API key to connect to private Crypto.com APIs.
        :param bitstamp_secret_key: The API secret.
        ...
        self._bitstamp_auth = BitstampAuth(bitstamp_api_key, bitstamp_secret_key)
        self._order_book_tracker = BitstampOrderBookTracker(trading_pairs=trading_pairs)
        self._ev_loop = asyncio.get_event_loop()
        self._shared_client = None
        self._poll_notifier = asyncio.Event()
        self._last_timestamp = 0
        self._in_flight_orders = {}  # Dict[client_order_id:str, BitstampInFlightOrder]
    ...
    async def start_network(self):
        ...
        if self._trading_required:
            ...
            # Bitstamp does not have any User Data Websocket channels
    ...
    def _format_trading_rules(self, instruments_info: Dict[str, Any]) -> Dict[str, TradingRule]:
        ...
        for rule in instruments_info["result"]["instruments"]:
            try:
                trading_pair = bitstamp_utils.convert_from_exchange_trading_pair(rule["instrument_name"])
    ...
    async def _api_request(self,
        ...
        if is_auth_required:
            request_id = bitstamp_utils.RequestId.generate_request_id()
            data = {"params": params}
            params = self._bitstamp_auth.generate_auth_dict(path_url, request_id,
                                                            bitstamp_utils.get_ms_timestamp(), data)
            headers = self._bitstamp_auth.get_headers()
        else:
    ...
    def buy(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        """
        order_id: str = bitstamp_utils.get_new_client_order_id(True, trading_pair)
    ...
    def sell(self, trading_pair: str, amount: Decimal, order_type=OrderType.MARKET,
        ...
        :returns A new internal order id
        """
        order_id: str = bitstamp_utils.get_new_client_order_id(False, trading_pair)
    ...
    async def _create_order(self,
        ...
        api_params = {"instrument_name": bitstamp_utils.convert_to_exchange_trading_pair(trading_pair),
    ...
    def start_tracking_order(self,
        ...
        """
        self._in_flight_orders[order_id] = BitstampInFlightOrder(
    ...
    async def _execute_cancel(self, trading_pair: str, order_id: str) -> str:
        ...
        try:
            ...
            await self._api_request(
                ...
                {"instrument_name": bitstamp_utils.convert_to_exchange_trading_pair(trading_pair),
    ...
    def _process_order_message(self, order_msg: Dict[str, Any]):
        ...
        if tracked_order.is_cancelled:
            ...
            self.logger().info(f"The market order {client_order_id} has failed according to order status API. "
                               f"Reason: {bitstamp_utils.get_api_reason(order_msg['reason'])}")
            self.trigger_event(MarketEvent.OrderFailure,
                               MarketOrderFailureEvent(
                                   self.current_timestamp,
                                   client_order_id,
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg="Could not fetch user events from Bitstamp. Check API key and network connection."
                )
    ...
    async def _user_stream_event_listener(self):
        ...
        BitstampAPIUserStreamDataSource.
        """
    ...
    async def get_open_orders(self) -> List[OpenOrder]:
        ...
        for order in result["result"]["order_list"]:
            if bitstamp_utils.HBOT_BROKER_ID not in order["client_oid"]:
                continue
            if order["type"] != "LIMIT":
            ...
            ret_val.append(
                OpenOrder(
                    ...
                    trading_pair=bitstamp_utils.convert_from_exchange_trading_pair(order["instrument_name"]),
