...
class BitstampInFlightOrder(InFlightOrderBase):
    def __init__(self,
                 client_order_id: str,
    ...
    def is_failure(self) -> bool:
        return NotImplementedError(f"{CONSTANTS.EXCHANGE_NAME} exchange does not have a failure order status.")
    ...
    def update_with_trade_update(self, trade_update: Dict[str, Any]) -> bool:
        """
        Updates the in flight order with trade update (from v2/order_status/ end point)
        return: True if the order gets updated otherwise False
        """
        trade_id = trade_update["transactions"]["tid"]
