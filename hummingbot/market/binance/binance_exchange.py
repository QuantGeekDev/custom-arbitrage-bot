...
def get_client_order_id(order_side, trading_pair):
...
class BinanceExchange(ExchangeBase):
    ...
    def convert_from_exchange_trading_pair(exchange_trading_pair: str) -> Optional[str]:
        if BinanceExchange.split_trading_pair(exchange_trading_pair) is None:
            return None
        # Binance does not split BASEQUOTE (BTCUSDT)
        base_asset, quote_asset = BinanceExchange.split_trading_pair(exchange_trading_pair)
    ...
    async def _update_trading_rules(self):
        ...
        current_tick = (self._current_timestamp / 60.0)
    ...
    async def _update_order_fills_from_trades(self):
        ...
        current_tick = (self._current_timestamp / self.UPDATE_ORDER_STATUS_MIN_INTERVAL)
            ...
            try:
                ...
                if event_type == "executionReport":
                    ...
                    if execution_type == "TRADE":
                        ...
                        order_filled_event = order_filled_event._replace(trade_fee=self.get_fee(
                            tracked_order.base_asset,
                            tracked_order.quote_asset,
    ...
    def buy(self, trading_pair, amount, order_type=OrderType.LIMIT, price=s_decimal_NaN,
              kwargs={}):
        t_pair = BinanceExchange.convert_from_exchange_trading_pair(trading_pair)
        order_id = get_client_order_id("buy", t_pair)
    ...
    def sell(self, trading_pair, amount, order_type=OrderType.LIMIT, price=s_decimal_NaN,
                 kwargs={}):
        t_pair = BinanceExchange.convert_from_exchange_trading_pair(trading_pair)
        order_id = get_client_order_id("sell", t_pair)
    ...
    def stop_tracking_order(self, order_id):
    ...
    def get_order_size_quantum(self, trading_pair, order_size):
        trading_rule = self._trading_rules[trading_pair]
        return Decimal(trading_rule.min_base_amount_increment)

    def quantize_order_amount(self, trading_pair, amount, price=s_decimal_0):
        trading_rule = self._trading_rules[trading_pair]
