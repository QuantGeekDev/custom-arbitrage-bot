    ...
    cdef c_apply_budget_constraint(self, object proposal):
        ...
        for buy in proposal.buys:
            ...
            if quote_balance < quote_size_total + quote_size:
                if not self._lower_order_amount:
                    ...
                    adjusted_amount = quote_balance/(buy.price * (Decimal('1') + buy_fee))
                    adjusted_amount = market.c_quantize_order_amount(self.trading_pair, adjusted_amount)
                    self.logger().info(f"Adjusted order amount: Buy order (price: {buy.price}, size: {buy.size}). Adjusted size: {adjusted_amount}).")
        ...
        for sell in proposal.sells:
            ...
            if base_balance < base_size_total + base_size:
                if not self._lower_order_amount:
                    ...
                    self.logger().info(f"Adjusting order amount: Sell order (price: {sell.price}, original size: {sell.size}. Adjusted size: {adjusted_amount}).")
